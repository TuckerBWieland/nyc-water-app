...
import { handleAsyncOperation, ErrorSeverity, ErrorContext, AsyncResult } from '../utils/errorHandler';
  ...
  const fetchForDate = async (date: string): Promise<AsyncResult<GeoJSONCollection>> => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        const fetchError = new Error(`Failed to load data: ${response.status} ${response.statusText}`);
