 ...
 */
export async function findNearestRainfallStation(lat: number, lon: number): Promise<RainfallStation | null> {
  try {
 ...
 */
export async function get7DayRainfallTotal(stationId: string, sampleDate: string | Date): Promise<number | null> {
  try {
 ...
 * @returns Promise resolving to 7-day total rainfall in mm or null if data unavailable
 */
export async function getOpen7DayRainfallTotal(lat: number, lon: number, sampleDate: string | Date): Promise<number | null> {
  try {
    // Parse the sample date with explicit UTC handling
    const endDate = parseDateUTC(sampleDate);
    ...
    if (!data.daily || !data.daily.precipitation_sum) {
      console.warn('No precipitation data available from Open-Meteo for the given location and dates');
      return null;
    }
