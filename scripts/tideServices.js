...
async function fetchTideStations() {
  ...
  try {
    ...
    const tideStations = data.stations.filter(station => station.tidal && station.tidal === true);
...
export async function findNearestTideStation(lat, lon) {
  try {
    ...
    console.log(
      `Nearest tide station: ${nearest.name} (${nearest.id}), ${nearest.distance.toFixed(
        2
      )} km away`
    );
    return nearest;
