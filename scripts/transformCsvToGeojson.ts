...
// Define the expected structure of CSV rows
interface CsvRow {
  'Site Name': string;
  Latitude: string;
  Longitude: string;
 ...
 */
function updateIndexFile(availableDates: string[]): void {
 ...
 */
function processAllCsvFiles(): void {
  ...
  const existingDates = fs.readdirSync(GEOJSON_DIR)
