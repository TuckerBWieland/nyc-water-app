...
function findColumn(columns, possibilities) {
  for (const possibility of possibilities) {
    const match = columns.find(col => col.toLowerCase() === possibility.toLowerCase());
 ...
 */
export async function processCsvDirectory(
  csvDir = DEFAULT_CSV_DIR,
  outputDir = DEFAULT_GEOJSON_DIR
) {
  try {
    ...
    const files = fs
      .readdirSync(csvDir)
      .filter(file => file.endsWith('.csv') || file.endsWith('.CSV'));
