 ...
 */
export async function findNearestRainfallStation(lat, lon) {
  try {
 ...
 */
export function findNearestStationWithinRadius(stations, lat, lon, maxDistanceKm) {
 ...
 */
export function formatDateForRainfall(date) {
 ...
 */
export async function get7DayRainfallTotal(stationId, sampleDate) {
  try {
 ...
 * @returns Promise resolving to 7-day total rainfall in mm or null if data unavailable
 */
export async function getOpen7DayRainfallTotal(lat, lon, sampleDate) {
  try {
    // Parse the sample date with explicit UTC handling
    const endDate = parseDateUTC(sampleDate);
    // Calculate the start date (7 days before the sample date)
